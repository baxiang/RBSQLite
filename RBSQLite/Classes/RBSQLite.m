...
if ([RBSQLUtils checkStringIsEmpty:tableName]) {                       \
NSLog(@" \n Fail!Fail!Fail!Fail! \n with TableName is nil"); \
return NO;                                                        \
}

#define LKDBCode_Async_Begin             \
__LKDBWeak RBSQLite *wself = self; \
[self asyncBlock :^{__strong RBSQLite *sself = wself;           \
if (sself) {
...
if ([model.class getModelInfos].count == 0) {                                                  \
NSLog(@"class: %@  property count is 0!!", NSStringFromClass(model.class));           \
return NO;                                                                                 \
}                                                                                              \
NSString *_model_tableName = model.db_tableName ?: [model.class getTableName];                 \
if ([RBSQLUtils checkStringIsEmpty:_model_tableName]) {                                         \
NSLog(@"model class name %@ table name is invalid!", NSStringFromClass(model.class)); \
return NO;                                                                                 \
}
@interface RBSQLiteWeakObject : NSObject
@property (nonatomic, LKDBWeak) RBSQLite *obj;
@end

@implementation RBSQLiteWeakObject
...
@property (nonatomic, strong) NSMutableArray *createdTableNames;//创建的表名称
@property (nonatomic, strong) NSRecursiveLock *threadLock;
@end
@implementation RBSQLite
...
- (instancetype)init
{
    return [self initWithDBName:@"RBSQLite"];
}
...
- (instancetype)initWithDBPath:(NSString *)filePath
{
    ...
    if (helper) {
        ...
        if (self) {
            ...
            [RBSQLite dbHelperWithPath:nil save:self];
        }
    }
...
+ (NSString *)getDBPathWithDBName:(NSString *)dbName
{
    ...
    NSString *filePath = [RBSQLUtils getPathForDocuments:fileName inDir:@"db"];
    return filePath;
}
+ (RBSQLite *)dbHelperWithPath:(NSString *)dbFilePath save:(RBSQLite *)helper
{
    NSMutableArray *dbArray = [self dbHelperSingleArray];
    RBSQLite *instance = nil;
    @synchronized(dbArray)
    {
        if (helper) {
            RBSQLiteWeakObject *weakObj = [[RBSQLiteWeakObject alloc] init];
            weakObj.obj = helper;
            [dbArray addObject:weakObj];
        }
        else if (dbFilePath) {
            for (NSInteger i = 0; i < dbArray.count;) {
                RBSQLiteWeakObject *weakObj = [dbArray objectAtIndex:i];
                if (weakObj.obj == nil) {
...
- (id)modelValueWithProperty:(RBSQLProperty *)property model:(NSObject *)model
{
    id value = nil;
...
- (BOOL)updateToDBBase:(NSObject *)model where:(id)where
{
    ...
    RBSQLModel *infos = [modelClass getModelInfos];
    ...
    for (NSInteger i = 0; i < infos.count; i++) {
        RBSQLProperty *property = [infos objectWithIndex:i];
        if ([RBSQLUtils checkStringIsEmpty:property.sqlColumnName]) {
            continue;
        }
    ...
    // 添加where 语句
    if ([where isKindOfClass:[NSString class]] && ([RBSQLUtils checkStringIsEmpty:where] == NO)) {
        ...
        if (pwhere.length == 0) {
            NSLog(@"database update fail : %@ no find primary key!", NSStringFromClass(modelClass));
            return NO;
        }
...
{
    RBSQLModel *infos = [model.class getModelInfos];
    ...
    if (primaryKeys.count > 0) {
        for (NSInteger i = 0; i < primaryKeys.count; i++) {
            ...
            if ([RBSQLUtils checkStringIsEmpty:pk] == NO) {
                RBSQLProperty *property = [infos objectWithSqlColumnName:pk];
                id pvalue = nil;
                
                if (property && [property.type isEqualToString:RBSQL_Map_UserCalculate]) {
...
- (void)executeForTransaction:(BOOL (^)(RBSQLite *))block
{
    RBSQLite *helper = self;
...
{
    [[modelClass defaultSQLite] executeDB:^(FMDatabase *db) {
...
- (BOOL)insertBase:(NSObject *)model
{
    ...
    RBSQLProperty *primaryProperty = [model singlePrimaryKeyProperty];
    NSString * primaryKey = [[model class] getPrimaryKey];
    id primaryValue;
    for (NSInteger i = 0; i < infos.count; i++) {
        RBSQLProperty *property = [infos objectWithIndex:i];
        if ([RBSQLUtils checkStringIsEmpty:property.sqlColumnName]) {
        ...
        if ([property isEqual:primaryProperty]) {
            if ([property.sqlColumnType isEqualToString:RBSQL_Type_Int] && [model singlePrimaryKeyValueIsEmpty]) {
                continue;
            }
        }
        id value = [self modelValueWithProperty:property model:model];
        ...
        if ([property.sqlColumnName isEqualToString:primaryKey]) {
            primaryValue = value;
        }
   ...
   __block NSString *insertSQL = [NSString stringWithFormat:@"replace into %@(%@) values(%@)", db_tableName, insertKey, insertValuesString];
    ...
    [self executeDB:^(FMDatabase *db) {
        NSString *select = [NSString stringWithFormat:@"SELECT * from %@ WHERE %@=%@", db_tableName,primaryKey,primaryValue];
        FMResultSet *set = [db executeQuery:select];
        if (([set columnCount] > 0) && [set next]) {
            
            insertSQL = [NSString stringWithFormat:@"update %@ set %@ where %@ = %@", db_tableName, updateKey,primaryKey,primaryValue];
        }else{
            insertSQL = [NSString stringWithFormat:@"insert into %@(%@) values(%@)", db_tableName, insertKey, insertValuesString];
        }
...
{
    [self executeDB:^(FMDatabase *db) {
        RBSQLModel *infos = [clazz getModelInfos];
        ...
        for (NSInteger i = 0; i < infos.count; i++) {
            ...
            if ([columnArray containsObject:property.sqlColumnName.lowercaseString] == NO) {
                ...
                if ((property.length > 0) && [property.sqlColumnType isEqualToString:RBSQL_Type_Text]) {
                    [addColumePars appendFormat:@"(%ld)", (long)property.length];
                }
                
                if (property.isNotNull) {
                    [addColumePars appendFormat:@" %@", RBSQL_Attribute_NotNull];
                }
                
                if (property.checkValue) {
                    [addColumePars appendFormat:@" %@(%@)", RBSQL_Attribute_Check, property.checkValue];
                }
                
                if (property.defaultValue) {
                    [addColumePars appendFormat:@" %@ %@", RBSQL_Attribute_Default, property.defaultValue];
                }
                NSString *alertSQL = [NSString stringWithFormat:@"alter table %@ add column %@", tableName, addColumePars];
                NSString *defaultValue = @"0";
                if ([property.sqlColumnType isEqualToString:RBSQL_Type_Text]) {
                    if (LKDBNullIsEmptyString) {
                        defaultValue = @"";
                    }
                    else {
...
- (BOOL)_createTableWithModelClass:(Class)modelClass tableName:(NSString *)tableName
{
    ...
    if (infos.count == 0) {
        NSLog(@"Class: %@ 0属性 不需要创建表", NSStringFromClass(modelClass));
        return NO;
    }
    ...
    for (NSInteger i = 0; i < infos.count; i++) {
        ...
        if (property.isNotNull) {
            [table_pars appendFormat:@" %@", RBSQL_Attribute_NotNull];
        }
        
        if (property.isUnique) {
            [table_pars appendFormat:@" %@", RBSQL_Attribute_Unique];
        }
        
        if (property.checkValue) {
            [table_pars appendFormat:@" %@(%@)", RBSQL_Attribute_Check, property.checkValue];
        }
        
        if (property.defaultValue) {
            [table_pars appendFormat:@" %@ %@", RBSQL_Attribute_Default, property.defaultValue];
        }
...
- (NSMutableArray *)searchWithParams:(RBSQLQueryParams *)params
{
    if (params.callback) {
        LKDBCode_Async_Begin;
...
{
    if ([RBSQLUtils checkStringIsEmpty:groupBy] == NO) {
        [sql appendFormat:@" group by %@", groupBy];
    }
    
    if ([RBSQLUtils checkStringIsEmpty:orderby] == NO) {
...
- (NSMutableArray *)searchBaseWithParams:(RBSQLQueryParams *)params
{
    if (params.toClass == nil) {
        NSLog(@"you search pars:%@! \n toClass is nil", params.getAllPropertysString);
        return nil;
    }
    
    NSString *db_tableName = params.tableName;
    
    if ([RBSQLUtils checkStringIsEmpty:db_tableName]) {
    ...
    else if ([RBSQLUtils checkStringIsEmpty:params.columns] == NO) {
    ...
    else if ([RBSQLUtils checkStringIsEmpty:params.where] == NO) {
...
- (void)search:(Class)modelClass where:(id)where orderBy:(NSString *)orderBy offset:(NSInteger)offset count:(NSInteger)count callback:(void (^)(NSMutableArray *))block
{
    ...
    LKDBCode_Async_Begin;
    RBSQLQueryParams *params = [[RBSQLQueryParams alloc] init];
    ...
    if (!modelClass) {
        ...
        while ([set next]) {
            ...
            for (int i = 0; i < columnCount; i++) {
                ...
                RBSQLProperty *property = [infos objectWithSqlColumnName:sqlName];
                ...
                if (isRowid && ((property == nil) || [property.sqlColumnType isEqualToString:RBSQL_Type_Int])) {
                    bindingModel.rowid = [set longForColumnIndex:i];
                }
                else {
                    BOOL isUserCalculate = [property.type isEqualToString:RBSQL_Map_UserCalculate];
