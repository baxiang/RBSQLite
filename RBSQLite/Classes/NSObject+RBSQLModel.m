...
#endif
@implementation NSObject (RBSQLModel)

+ (RBSQLite *)defaultSQLite{
    static RBSQLite *helper;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        helper = [[RBSQLite alloc] init];
    });
    return helper;
}
...
{
    objc_setAssociatedObject(self, @selector(db_tableName), db_tableName, OBJC_ASSOCIATION_COPY_NONATOMIC);
}
- (NSString *)db_tableName
{
    NSString *tableName = objc_getAssociatedObject(self, _cmd);
...
{
    objc_setAssociatedObject(self, @selector(rowid), [NSNumber numberWithInteger:rowid], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (NSInteger)rowid
{
...
{
    RBSQLModel *infos = [self.class getModelInfos];
...
- (void)setDb_inserting:(BOOL)db_inserting
{
    ...
    objc_setAssociatedObject(self, @selector(db_inserting), number, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
+ (void)columnAttributeWithProperty:(RBSQLProperty *)property
{
    //overwrite
}
- (void)userSetValueForModel:(RBSQLProperty *)property value:(id)value
{
}
+ (RBSQLModel *)getModelInfos
{
    ...
    if (infos == nil) {
        ...
        if ([self isContainParent] && [self superclass] != [NSObject class]) {
            RBSQLModel *superInfos = [[self superclass] getModelInfos];
            for (NSInteger i = 0; i < superInfos.count; i++) {
                RBSQLProperty *db_p = [superInfos objectWithIndex:i];
        ...
        if (pronames.count > 0) {
            infos = [[RBSQLModel alloc] initWithKeyMapping:keymapping propertyNames:pronames propertyType:protypes primaryKeys:pkArray];
        }
        else {
            infos = [[RBSQLModel alloc] init];
        }
...
+ (void)getSelfPropertys:(NSMutableArray *)pronames protypes:(NSMutableArray *)protypes
{
    ...
    for (i = 0; i < outCount; i++) {
        ...
        if ([RBSQLUtils checkStringIsEmpty:propertyClassName]) {
            ///没找到具体的属性就放弃
            continue;
        }
        ///添加属性
...
- (id)userGetValueForModel:(RBSQLProperty *)property
{
    return nil;
}
...
- (id)modelGetValue:(RBSQLProperty *)property
{
    ...
    if (value == nil) {
        ...
        returnValue = [[RBSQLUtils numberFormatter] stringFromNumber:value];
    }
        ...
        if (formatter) {
            ...
            returnValue = [RBSQLUtils stringWithDate:value];
        }
...
- (id)db_jsonObjectFromDictionary:(NSDictionary *)dic
{
    if ([NSJSONSerialization isValidJSONObject:dic]) {
        NSDictionary *bomb = @{ RBSQL_TypeKey : RBSQL_TypeKey_JSON, RBSQL_ValueKey : dic };
        return bomb;
    }
    else {
        ...
        if (toDic.count) {
            NSDictionary *bomb = @{ RBSQL_TypeKey : RBSQL_TypeKey_Combo, RBSQL_ValueKey : toDic };
            return bomb;
        }
    }
    return nil;
...
- (NSDictionary *)db_getPrimaryKeysValues
{
    ...
    [array enumerateObjectsUsingBlock:^(NSString *pname, NSUInteger idx, BOOL * _Nonnull stop) {
        RBSQLProperty *property = [infos objectWithSqlColumnName:pname];
        id value = nil;
        if ([property.type isEqualToString:RBSQL_Map_UserCalculate]) {
...
- (NSDictionary *)db_readInfoWithModel:(NSObject *)model class:(Class)clazz
{
    ...
    jsonObject[RBSQL_TypeKey] = RBSQL_TypeKey_Model;
    jsonObject[RBSQL_TableNameKey] = model.db_tableName;
    jsonObject[RBSQL_ClassKey] = NSStringFromClass(clazz);
    jsonObject[RBSQL_RowIdKey] = @(model.rowid);
    
    NSDictionary *dic = [model db_getPrimaryKeysValues];
    if (dic.count > 0 && [NSJSONSerialization isValidJSONObject:dic]) {
        jsonObject[RBSQL_PValueKey] = dic;
    }
    return jsonObject;
}
...
- (id)db_jsonObjectWithObject:(id)obj
{
    ...
    if ([obj isKindOfClass:[NSString class]] || [obj isKindOfClass:[NSNumber class]]) {
        ...
        if (formatter) {
            ...
            dateString = [RBSQLUtils stringWithDate:obj];
        }
        dateString = [dateString stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
        if (dateString.length > 0) {
            jsonObject = @{ RBSQL_TypeKey : RBSQL_TypeKey_Date, RBSQL_ValueKey : dateString };
        }
    }
...
- (id)db_jsonObjectFromArray:(NSArray *)array
{
    if ([NSJSONSerialization isValidJSONObject:array]) {
        NSDictionary *bomb = @{ RBSQL_TypeKey : RBSQL_TypeKey_JSON, RBSQL_ValueKey : array };
        return bomb;
    }
    else {
        ...
        if (toArray.count) {
            NSDictionary *bomb = @{ RBSQL_TypeKey : RBSQL_TypeKey_Combo, RBSQL_ValueKey : toArray };
            return bomb;
        }
    }
    return nil;
...
+ (NSString *)getDBDataPathWithName:(NSString *)filename
{
    ...
    return [RBSQLUtils getPathForDocuments:filename inDir:dir];
}
...
{
    RBSQLProperty *property = [self singlePrimaryKeyProperty];
    if (property) {
        if ([property.type isEqualToString:RBSQL_Map_UserCalculate]) {
...
- (BOOL)singlePrimaryKeyValueIsEmpty
{
    ...
    if (property) {
        ...
        if ([property.sqlColumnType isEqualToString:RBSQL_Type_Int]) {
            if ([pkvalue isKindOfClass:[NSString class]]) {
                if ([RBSQLUtils checkStringIsEmpty:pkvalue])
                    return YES;
...
- (void)modelSetValue:(RBSQLProperty *)property value:(NSString *)value
{
    ...
    if (columnClass == nil) {
        ...
        NSString *columnType = property.propertyType;
        if ([RBSQL_Convert_FloatType rangeOfString:columnType].location != NSNotFound) {
            if (value) {
                modelValue = [[RBSQLUtils numberFormatter] numberFromString:value];
            }
        }
        else if ([RBSQL_Convert_IntType rangeOfString:columnType].location != NSNotFound) {
            if (value) {
                modelValue = [[RBSQLUtils numberFormatter] numberFromString:value];
            }
        }
        ...
        if (formatter) {
            ...
            modelValue = [RBSQLUtils dateWithString:datestr];
        }
    }
...
- (id)db_objectWithDictionary:(NSDictionary *)dic
{
    ...
    NSString *type = [dic objectForKey:RBSQL_TypeKey];
    if (type) {
        if ([type isEqualToString:RBSQL_TypeKey_Model]) {
            Class clazz = NSClassFromString([dic objectForKey:RBSQL_ClassKey]);
            NSInteger rowid = [[dic objectForKey:RBSQL_RowIdKey] integerValue];
            NSString *tableName = [dic objectForKey:RBSQL_TableNameKey];
            
            NSString *where = nil;
            
            NSString *rowCountWhere = [NSString stringWithFormat:@"select count(rowid) from %@ where rowid=%ld limit 1", tableName, (long)rowid];
            NSInteger result = [[[clazz defaultSQLite] executeScalarWithSQL:rowCountWhere arguments:nil] integerValue];
            if (result > 0) {
                where = [NSString stringWithFormat:@"select rowid,* from %@ where rowid=%ld limit 1", tableName, (long)rowid];
            }
            else {
                NSDictionary *pv = [dic objectForKey:RBSQL_PValueKey];
                if (pv.count > 0) {
            ...
            if (where) {
                NSArray *array = [[clazz defaultSQLite] searchWithSQL:where toClass:clazz];
                if (array.count > 0) {
        ...
        else if ([type isEqualToString:RBSQL_TypeKey_JSON]) {
            id value = [dic objectForKey:RBSQL_ValueKey];
            return value;
        }
        else if ([type isEqualToString:RBSQL_TypeKey_Combo]) {
            id value = [dic objectForKey:RBSQL_ValueKey];
            if ([value isKindOfClass:[NSArray class]]) {
        ...
        else if ([type isEqualToString:RBSQL_TypeKey_Date]) {
            NSString *datestr = [dic objectForKey:RBSQL_ValueKey];
            NSDateFormatter *formatter = [self.class getModelDateFormatter];
            if (formatter) {
                return [formatter dateFromString:datestr];
            }
            else {
                return [RBSQLUtils dateWithString:datestr];
            }
        }
    }
    else {
